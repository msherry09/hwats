...
<html>
	...
	function validateForm()
	{
		...
		//set regex variables
		var emailformat=/^([\w-\.]+)@[\w.-]+[.][a-zA-Z0-9]+/; //regular expression defining email format
		...
		document.getElementById("othermedError").innerHTML= "";
		...
		if(document.getElementById('otherMed').checked == true && document.getElementById('othertext').value == "")
		{
			if(focusString == "none")
				focusString = "othertext";
			numErrors++;
			document.getElementById("othermedError").innerHTML= "Required field";
		}
...
//-->
	//Function for disabling and enabling textfield for Other medication
	...
	<body>
		<div class="site"><!-- SITE CONTAINER -->
			...
			<div class="container container_16">
				...
				<form action="post/demographicformpost.php" name='demographicForm' id='demoForm' method="post" onSubmit='return validateForm()'>
				...
				<div class='clear'><input type='hidden' id='partid' name='partid' value='<?php echo $partid; ?>' /></div>
					<fieldset>
						...
						<div class='grid_11'>
							<p><input type="text" id="address" name="address" maxlength="50" <?php if($partid != 0){echo "value='".$row_participant[4]."'";} ?>/></p>
						</div>
					...
					<fieldset>
						...
						<div class='grid_14 prefix_1'>
							...
							<input <?php if($partid != 0) { if($row_participant[22] == 1) { echo ' checked=\'true\' '; } } ?> type="checkbox" name="pain" value="1" /> Pain<br />
							<input <?php if($partid != 0) { if($row_participant[23] == 1) { echo ' checked=\'true\' '; } } ?> type="checkbox" name="heart" value="1" /> Heart Issues&nbsp;&nbsp;&nbsp;
							<input <?php if($partid != 0) { if($row_participant[24] == 1) { echo ' checked=\'true\' '; } } ?> type="checkbox" name="blood" value="1" /> Blood Pressure&nbsp;&nbsp;&nbsp;
							<input <?php if($partid != 0) { if($row_participant[25] == 1) { echo ' checked=\'true\' '; } } ?> type="checkbox" name="cholesterol" value="1" /> Cholesterol&nbsp;&nbsp;&nbsp;
							<input <?php if($partid != 0) { if($row_participant[26] != 0) { echo ' checked=\'true\' '; } } ?> type="checkbox" name="other" id="otherMed" value="1" onClick="enableText(this.checked)" /> Other&nbsp;&nbsp;&nbsp;
							<?php
								//get info from the othermed table
								$sql_othermed = "select * from othermed where participantid = ".$partid;
								$result_othermed = mysql_query($sql_othermed);
								$row_othermed = mysql_fetch_row($result_othermed);								
							?>
							<input type="text" maxlength="50" id="othertext" name="othertext" <?php if($partid != 0){echo "value='".$row_othermed[2]."'";} if($row_othermed[2] == ''){echo "disabled='true'";}?> />
							<span class="errortext" id="othermedError"></span></p>
						</div>						
						...
						<?php
							if($partid != 0){
								$wwsql = "SELECT WWID, WWDESC FROM wwstatuslookup ORDER BY WWID";
								$wwresult = mysql_query($wwsql);
						...
						<div class='grid_2 suffix_7'>
							<p><select id="weightwatchers" name="weightwatchers">
								<?php
								while($row = mysql_fetch_array($wwresult))
								{
									$wwid = $row['WWID'];
									echo "<option ".($partid != 0 ? ($wwid == $row_participant[28] ? " selected " : "") : "")." value=\"".$row['WWID']."\">".$row['WWDESC']."</option>\n  ";
								}
								?>
							</select></p>
						</div>
					...
					<div class='clear'>&nbsp;</div>
					<div class='grid_16'>
						<p class='left blocktext'><b><u>What we will do with the information we obtain:</u></b></p>
					</div>
					<div class='clear'>&nbsp;</div>
					<div class='grid_16'>
						<p class="blocktext">We will be collecting all the information in order to determine your level of risk for diabetes and weight control issues. The results we obtain today will allow us to provide you with immediate education "coaching" today. A trained health professional will review all the results with you and establish future plans based on your specific needs.</p>
					</div>
					<div class='clear'>&nbsp;</div>
					<div class='grid_16'>
						<p class='left blocktext'><b><u>Ongoing communication with you:</u></b></p>
					</div>
					<div class='clear'>&nbsp;</div>
					<div class='grid_16'>
						<p class="blocktext">You will receive ongoing communication from us. The frequency and detail of communication will be based on the risk factors from today's assessment. In general, the higher your risk factors, the sooner we will be communicating with you.</p>
					</div>
					<div class='clear'>&nbsp;</div>
					<div class='grid_16'>
						<p class='left blocktext'><b><u>All services are free of charge</u></b></p>
					</div>
					<div class='clear'>&nbsp;</div>
					<div class='grid_16'>
						<p class='left blocktext'><b><u>Future services may have fees/charges:</u></b></p>
					</div>
					<div class='clear'>&nbsp;</div>
					<div class='grid_16'>
						<p class="blocktext">For all future services related to this project and your participation, we will always inform you in advance if the service is
					...
					<div class='clear'>&nbsp;</div>
					<div class="buttonarea"><input type="submit" value="Submit"/>
					<input type='button' value='Cancel' onclick='window.location="landingpage.php"' /></div>
				</form>
			</div>
		<!-- END PAGE CONTENT -->
